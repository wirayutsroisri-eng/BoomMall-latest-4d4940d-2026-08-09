/**
 * Auto-merge same-address orders for Boom Mall shipping labels.
 * Key = same shop + recipient name + phone + shipping address.
 */
...
export function mergeSameAddressOrders(orders: MergeableOrder[]): MergedShipment[] {
  ...
  groups.sort(
    (a, b) =>
      ...
      (a.recipient.name ?? '').localeCompare(b.recipient.name ?? '', 'th'),
  );
  return groups;
}
